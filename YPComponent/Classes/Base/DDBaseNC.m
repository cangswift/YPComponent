...
@interface DDBaseNC ()<UIGestureRecognizerDelegate,UINavigationControllerDelegate>

@property (nonatomic , assign) id systemPanTarget; //系统侧滑返回代理对象
@property (nonatomic , assign) SEL systemPanHandler; //系统侧滑返回回调

@property (nonatomic , strong) DDBaseVC *curVC; //用于记录侧滑返回的当前视图控制器
@property (nonatomic , strong) DDBaseVC *preVC; //用于记录侧滑返回的前一个视图控制器

@property (nonatomic, getter=isPushing) BOOL pushing; //记录push状态，防止多次push

@end

@implementation DDBaseNC
...
- (void)backPanHandler:(UIPanGestureRecognizer *)pan{
    if(_systemPanTarget && _systemPanHandler){
        ...
        if(pan.state == UIGestureRecognizerStateBegan){
            ...
            _curVC = [tmpCurVC isKindOfClass:[DDBaseVC class]] ? (DDBaseVC *)tmpCurVC : nil;
            _preVC = [tmpPreVC isKindOfClass:[DDBaseVC class]] ? (DDBaseVC *)tmpPreVC : nil;
...
- (BOOL)gestureRecognizerShouldBegin:(UIPanGestureRecognizer *)gestureRecognizer {
    ...
    if([lastVC isKindOfClass:[DDBaseVC class]]){
        DDBaseVC *vc = (DDBaseVC *)lastVC;
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated{
    ...
    UIViewController *lastVC = self.viewControllers.lastObject;
    if([lastVC isKindOfClass:[DDBaseVC class]]){
        DDBaseVC *lvc = (DDBaseVC *)lastVC;
        self.alpha = lvc.navigationBarAlpha;
        if(viewController && [viewController isKindOfClass:[DDBaseVC class]]){
            DDBaseVC *vc = (DDBaseVC *)viewController;
...
- (NSArray *)popToViewController:(UIViewController *)viewController animated:(BOOL)animated{
    if([viewController isKindOfClass:[DDBaseVC class]]){
        DDBaseVC *vc = (DDBaseVC *)viewController;
        [UIView animateWithDuration:UINavigationControllerHideShowBarDuration
                         animations:^{
                             self.alpha = vc.navigationBarAlpha;
